...
describe('businessHoursBetween', () => {
  it('counts the hours inside one working day', () => {
    expect(
      businessHoursBetween(
        ict('2026-09-21T09:00'),
        ict('2026-09-21T10:30'),
        NONE,
      ),
    ).toBe(1.5);
  });
  ...
  it('counts nothing outside the window', () => {
    expect(
      businessHoursBetween(
        ...
        ict('2026-09-22T08:30'),
        NONE,
      ),
    ).toBe(0);
    expect(
      businessHoursBetween(
        ict('2026-09-21T05:00'),
        ict('2026-09-21T08:30'),
        NONE,
      ),
    ).toBe(0);
  });
  ...
  it('skips a listed holiday as if it were a weekend day', () => {
    const monday = new Set(['2026-09-21']);
    expect(
      businessHoursBetween(
        ict('2026-09-18T16:00'),
        ict('2026-09-22T09:30'),
        monday,
      ),
    ).toBe(1.5);
  });
  ...
  it('is zero when the range is empty or backwards', () => {
    ...
    expect(businessHoursBetween(at, at, NONE)).toBe(0);
    expect(businessHoursBetween(ict('2026-09-21T10:00'), at, NONE)).toBe(0);
  });
...
describe('addBusinessHours', () => {
  it('lands inside the same day when there is room', () => {
    expect(addBusinessHours(ict('2026-09-21T09:00'), 2, NONE)).toEqual(
  ...
  it('rolls the remainder into the next working day', () => {
    ...
    expect(addBusinessHours(ict('2026-09-18T15:30'), 2, NONE)).toEqual(
      ict('2026-09-21T09:30'),
    );
  });

  it('ends a fully used day at its close, not at the next opening', () => {
    expect(addBusinessHours(ict('2026-09-21T08:30'), 8, NONE)).toEqual(
      ict('2026-09-21T16:30'),
    );
  });
  ...
  it('puts 24 business hours from a Sunday 02:00 at Wednesday close', () => {
    expect(
      addBusinessHours(ict('2026-09-20T02:00'), EXPIRY_BUSINESS_HOURS, NONE),
    ).toEqual(ict('2026-09-23T16:30'));
  });

  it('steps over a holiday', () => {
    const tuesday = new Set(['2026-09-22']);
    // Mon 16:00 + 1 h: 0.5 h Monday, Tuesday is closed, 0.5 h Wednesday.
    expect(addBusinessHours(ict('2026-09-21T16:00'), 1, tuesday)).toEqual(
      ict('2026-09-23T09:00'),
    );
  });
  ...
  it('inverts businessHoursBetween', () => {
    ...
    for (const start of [
      ...
      const to = addBusinessHours(from, 13.25, holidays);
      expect(businessHoursBetween(from, to, holidays)).toBeCloseTo(13.25, 9);
    }
  });
});
...
describe('requestExpiry (§10.4, §15.1)', () => {
  ...
  it('is a predicate of the clock reading, with time left on the way down', () => {
    const fresh = requestExpiry(submitted, ict('2026-09-21T10:00'), NONE);
  ...
  it('is not actionable exactly at 24 business hours', () => {
    ...
    const at = requestExpiry(submitted, ict('2026-09-24T09:00'), NONE);
    expect(at).toMatchObject({ expired: true, hoursLeft: 0 });
    const before = requestExpiry(submitted, ict('2026-09-24T08:59'), NONE);
    expect(before.expired).toBe(false);
  });

  it('does not count a weekend against the Request', () => {
    const friday = ict('2026-09-18T16:00');
    const monday = requestExpiry(friday, ict('2026-09-21T09:30'), NONE);
    expect(monday.expired).toBe(false);
    expect(monday.hoursLeft).toBeCloseTo(22.5, 9);
  });

  it('reports when it expires, on the same clock', () => {
    expect(requestExpiry(submitted, submitted, NONE).expiresAt).toEqual(
      ict('2026-09-24T09:00'),
    );
  });

  it('is more generous when more days are holidays', () => {
    const now = ict('2026-09-23T12:00');
    const plain = requestExpiry(submitted, now, NONE);
    const withHoliday = requestExpiry(submitted, now, new Set(['2026-09-22']));
    expect(withHoliday.hoursLeft).toBeGreaterThan(plain.hoursLeft);
  });
});
