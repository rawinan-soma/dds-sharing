...
describe.skipIf(!adminUrl || !appUrl)("GET /api/reviewer/queue (e2e, ticket #65)", () => {
  ...
  async function submitOne(): Promise<{ id: string; referenceNumber: string }> {
    ...
    const list = await fetch(`${baseUrl}/api/reviewer/queue`, { headers: { cookie: `reviewer_session=${sessionCookie}` } });
    const { requests } = await list.json();
    const row = requests.find((r: { referenceNumber: string }) => r.referenceNumber === referenceNumber);
    return { id: row.id, referenceNumber };
  }

  async function csrfHeaders(): Promise<Record<string, string>> {
    const csrfRes = await fetch(`${baseUrl}/api/reviewer/csrf`, { headers: { cookie: `reviewer_session=${sessionCookie}` } });
    const { csrfToken } = (await csrfRes.json()) as { csrfToken: string };
    const csrfCookie = extractCookie(csrfRes, "reviewer_csrf")!;
    return {
      "content-type": "application/json",
  ...
  it("refuses approve and reject without the CSRF token, and without a live session", async () => {
    ...
    const noSession = await fetch(`${baseUrl}/api/reviewer/queue/${id}/approve`, {
      method: "POST",
      headers: { ...headers, cookie: headers.cookie.replace(/reviewer_session=[^;]+/, "reviewer_session=not-a-real-session") },
    });
    expect(noSession.status).toBe(401);

    // Neither attempt should have moved the Request off pending.
    const detail = await fetch(`${baseUrl}/api/reviewer/queue/${id}`, { headers: { cookie: `reviewer_session=${sessionCookie}` } });
  ...
  it("approves a pending Request, moves it off the queue, and states plainly what was recorded", async () => {
    ...
    const res = await fetch(`${baseUrl}/api/reviewer/queue/${id}/approve`, { method: "POST", headers });
    expect(res.status).toBe(201);
    const body = await res.json();
    expect(body.outcome).toBe("approved");
    expect(body.decidedAt).toBeDefined();

    const stillListed = await fetch(`${baseUrl}/api/reviewer/queue`, { headers: { cookie: `reviewer_session=${sessionCookie}` } });
  ...
  it("refuses a reject with no internal note, and refuses approving an already-decided Request", async () => {
    ...
    const ok = await fetch(`${baseUrl}/api/reviewer/queue/${id}/reject`, {
      method: "POST",
      headers,
      body: JSON.stringify({ note: "Could not confirm this person's workplace by telephone." }),
    });
    expect(ok.status).toBe(201);
    expect((await ok.json()).outcome).toBe("rejected");

    const again = await fetch(`${baseUrl}/api/reviewer/queue/${id}/approve`, { method: "POST", headers });
