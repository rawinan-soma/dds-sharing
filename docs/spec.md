...
(`prototypes/request-lifecycle.prototype.html`).
...
Re-run, Snapshot, Disease group, Report code — mean exactly what it
...
as the other host commands. A procedure performed by hand is
...
- **Enforced by database roles, not by convention.** The application role holds
  `INSERT`/`SELECT` on the event tables and **`DELETE` nowhere**, and no role holds
  `UPDATE`. This is the one arrangement under which *"the running application
  cannot rewrite history"* is a fact rather than a promise. Convention-only was
  rejected explicitly: the record is permanent, so a bad `UPDATE` in year three
