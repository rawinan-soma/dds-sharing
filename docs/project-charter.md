...
>
> Every substantive claim below traces to [`spec.md`](spec.md) v1.0 (2026-09-02),
> to [`CONTEXT.md`](../CONTEXT.md), or to a closed ticket on
...
3. **The upstream API cannot be used naively — it is not merely slow, it is
   impossible to page through.** Measured (§5.3): a ~3.5 s fixed cost per request
   independent of rows returned; `OFFSET`-based pagination whose cost climbs with
   depth; and a **~60 s gateway timeout that makes pages past roughly 50
   unreachable**. A full-year group `02` extract is **1,141,658 rows = 115 pages**
   — a `504`, not a long wait. Every requester who tries hits this alone.
...
- **de-identification is a strict allowlist enforced in code** — 22 columns, never
  a judgement call;
...
3. **Extraction pipeline** — Probe, Fetch (contiguous half-open calendar months),
   Filter, Project, Completeness assert, retry/resume/stall handling (§5.4, §7).
4. **De-identification layer** — the six rules and the 22-column allowlist (§6).
...
16. [`spec.md`](spec.md) v1.0 — 2,297 lines, complete for implementation.
17. [`CONTEXT.md`](../CONTEXT.md) — the canonical glossary.
18. **ADRs 0001–0005** — the hard-to-reverse decisions and their reasoning.
...
- **De-identification to 22 allowlisted columns**, geography no finer than
  district (อำเภอ) (§6).
- **Live extraction from the upstream DDC DDS API**, date-chunked, sequential,
  global concurrency of 1 (§5, §7, §13.2).
...
| **Perform Defining Phase** | **✅ COMPLETE** — 28 decision tickets resolved (#2–#29), `CONTEXT.md`, ADRs 0001–0005, `spec.md` v1.0 | 2026-08-27 | 2026-09-02 |
| **Perform Measurement Phase** | Dev-cycle asks §17.2 — `birth_date` null rate, `diagnosis_icd10_list` delimiter, SMTP config, three test sends, relay hostname verbatim, wireframe | 2026-09-08 | 2026-09-19 |
...
| **Perform Control Phase** | Required tests §17.1; VM request; host firewalling; NTP; province seed; **large-download smoke test through the real ministry edge as a first-deploy gate**; Reviewer seeding ceremony; production deploy | 2026-11-03 | 2026-11-21 |
| **Deliver Project Summary Report and Close Out** | Handover, operator runbook, close map issue #1 | 2026-11-24 | 2026-11-28 |

> ⚠️ **The largest schedule risk sits in Control, not Improvement.** The 20–30 MB
> download through the real ministry edge is **the item most likely to break and
> the last one testable**, because no public route or ministry certificate exists
> before production (§17.4).
...
| **Improved Compliance** (การดำเนินงานปรับปรุงดีขึ้น) | Today: **no audit record of releases at all, and de-identification by hand.** After: 100% of releases carry a named Decision + Snapshot; de-identification is a code-enforced 22-column allowlist. **This is the project's principal benefit and it is qualitative — it goes from zero to complete.** | Not monetised |
...
|---|---|---|---|
| **R1** | **No PDPA §26 lawful basis and no DDC sign-off are on record** for releasing case-level DDS data to the open internet (§18.1). The approval gate is an **accountability record, not a lawful basis**. | Highest. Owned by the project manager. | Recorded as an assumption to be confirmed (A1). Reversal path: the kill switch — `docker compose down`, minutes (§17.4). |
...
| **R6** | **The large-download smoke test through the real ministry edge is untestable before production** (§17.4). No public hostname or ministry certificate exists during development. | Medium–high, schedule | First-deploy gate, exercised deliberately rather than discovered through a Requester. |
...
| **R11** | **Excel silently corrupts leading-zero geography codes and truncates above 1,048,576 rows** (§18.5, §18.6) — and Excel is a primary tool of the audience. A full-year group `02` extract exceeds the row limit. | Medium, accepted | Documented in the Data dictionary shipped in every archive. |
...
- **`OFFSET`-based pagination with a ~60 s gateway timeout** — pages past roughly
  50 are unreachable. Date-chunking is therefore **mandatory for correctness, not
  an optimisation**.
...
| # | Assumption | Status |
|---|---|---|
| **A1** | **The existing DDC data agreement, together with the human approval gate, covers the release of de-identified case-level DDS data.** | ⚠️ **To be confirmed with the PDPO.** `spec.md` §18.1 records that **no PDPO consultation was sought and no §26 lawful basis is recorded**, and that no DDC sign-off exists on releasing case-level DDS data to the open internet — by decision, not oversight. **This assumption is therefore unverified, and confirming it is the single highest-value action available to the sponsor.** If it does not hold, the reversal path is the kill switch (§17.4). |
| **A2** | The de-identification allowlist of 22 columns, with geography no finer than district, is sufficient to render the Extract non-personal for PDPA purposes. | ⚠️ To be confirmed with the PDPO. §18.2 records that the finest *effective* geography is narrower than the finest *named* geography. |
...
| **A6** | Upstream `total_items` is trustworthy as a completeness check. | Enforced: a chunk whose received count disagrees **fails the job and publishes nothing** (§7.5). |
...
| **A8** | Requesters read Thai and analyse in Excel, R or Python. | Drives §16.3 and the Excel caveats in §18.5–18.6. |
...
| **A10** | A ~20–30 MB archive traverses the ministry edge successfully. | **Untestable before production.** First-deploy gate (§17.4, R6). |
...
> **What signing this charter accepts.** Assumption **A1** is unverified: there is
> no PDPA §26 lawful basis and no DDC sign-off on record for releasing case-level
> DDS data to the open internet. Approving this charter proceeds on the
> assumption that the existing data agreement and the human approval gate suffice,
> and accepts risks **R1–R3** as stated.
