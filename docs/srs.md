...
| **Operator** | §2.4, §3, FR-22 to FR-25, FR-29 to FR-31, NFR-05, NFR-11, NFR-19 to NFR-23, NFR-31, NFR-32 |
...
| **PostgreSQL** | Outbound | The system of record. Application role: `INSERT`/`SELECT` on event tables, `DELETE` **nowhere** except `reviewer_session` and the login-throttle table; read-only on `province`. No role holds `UPDATE` on the event tables or on `request_contact` (§12.2, §15.4) |
...
- **Requester — at submit.** One Thai sentence — `requester_retention_notice`
  ...
  carrying three things: **what** is kept, that it is kept **indefinitely**, and
  **why** (audit and traceability of data releases). Naming the reason is what
  makes it read as a policy rather than a leak.
...
holds `UPDATE` on the event tables or on `request_contact`; only `reviewer_session`
and the login-throttle table are prunable, and the pruning job needs `DELETE` on
**exactly those two tables and on nothing else**.
...
| [#28](https://github.com/rawinan-soma/dds-sharing/issues/28) | Retention rule for personal data | FR-29, NFR-16, NFR-17 |
