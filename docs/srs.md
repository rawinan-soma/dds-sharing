...
Extract fingerprint** — mean exactly what
...
| **Repair it by hand** — Re-run, Resend, Reviewer account lifecycle, fingerprint verification, upstream traffic accounting — each a deliberate human act, several of them gated behind shell access | FR-26, FR-27, FR-29, FR-30, FR-31 |
...
> ⚠️ **Visual design is not settled here.** Structure, ordering and copy are
> settled. **Component choice and typeface are now settled — Tailwind + DaisyUI on
> Noto Sans Thai (§2.4.1).** Visual design and spacing still come from a wireframe
> the repo owner supplies during the dev cycle ([OQ-02](#64-open-questions)). Carry
> the two ordering rules above as requirements.
>
> **The prototype they were settled in no longer exists in the repo** — the
> `prototype/requester-reviewer-ui` branch was reverted out by `a76b2d1` and was
> never pushed. What it settled survives only as the prose above and in spec §16.4;
> there is no styling left to read as normative.
...
| `requester_retention_notice` | what is kept, that it is indefinite, and why |
...
password reset / TOTP re-enrolment / deactivation, Extract fingerprint
...
| **PostgreSQL** | Outbound | The system of record. Application role: `INSERT`/`SELECT` on event tables, `DELETE` **nowhere** except `reviewer_session` and the login-throttle table; read-only on `province`. No role holds `UPDATE` on `request` or `request_contact` (§12.2, §15.4) |
...
### 4.28 FR-28 — Removed — see [ADR 0019](adr/0019-a-requesters-contact-details-are-never-removed.md).
...
- Deactivation: `deactivated_at` set — **never a row deletion** — live sessions
  invalidated immediately by a Postgres query, and a `deactivated` event recording
  what happened to the Reviewer, never who ran the command
  ([ADR 0020](adr/0020-a-host-command-does-not-record-who-ran-it.md)). **Their
  open Alerts become clearable by any active Reviewer from that moment** (FR-25,
  ADR 0013); the assignment is not rewritten, and nothing about the Request stream
  is touched.
...
- **Requester — at submit.** One Thai sentence — `requester_retention_notice`
  ...
  carrying three
  things: **what** is kept, that it is kept **indefinitely**, and **why** (audit
  and traceability of data releases). Naming the reason is what makes it read as
  a policy rather than a leak.
...
holds `INSERT`/`SELECT` on the event tables and **`DELETE` nowhere**, and no role
holds `UPDATE`; only `reviewer_session` and the login-throttle table are
prunable, and the pruning job needs `DELETE` on **exactly those two tables and on
nothing else**.
...
| **R7** | **The audit record is permanent personal data, including about people who never used the service** — `token_lookup` rows matching no Request are IP addresses of anonymous strangers. | Medium, accepted | Kept deliberately: a token-guessing sweep is only visible in hindsight. §18.9, NFR-16 |
...
answered sixteen of the previous twenty, settled the implementation stack
(§2.4.1) and re-anchored the charter (OQ-06, closed). **None of these blocks building**; each has a decided rule
